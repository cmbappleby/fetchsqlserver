...
add_catvar_code_desc <- function(catvar_df) {
  ...
  for (i in seq_len(nrow(catvar_df))) {
    schema <- catvar_df$RefSchemaName[i]
    tbl <- catvar_df$RefTableName[i]

    qry <- paste0("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS ",
                  "WHERE TABLE_SCHEMA ='", schema, "'",
                  "AND TABLE_NAME = '", tbl, "'")

    tbl_cols <- DBI::dbGetQuery(conn, qry)

    catvar_df$Code[i] <- tbl_cols$COLUMN_NAME[1]
    catvar_df$Description[i] <- tbl_cols$COLUMN_NAME[2]
  }

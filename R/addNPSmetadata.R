...
gather_nps_info <- function(eml_info) {
  ...
  park_units_list <- get_overall_info("park_units")
  cat('Select the park units where the data were collected:\n')
  park_units <- get_user_response_multi(park_units_list)
  eml_info[['park_units']] <- park_units

  prod_units_list <- c(get_overall_info("network_code"), park_units)
  cat('Select the units responsible for generating the data package:\n')
  prod_units <- get_user_response_multi(prod_units_list)
  eml_info[['prod_units']] <- prod_units

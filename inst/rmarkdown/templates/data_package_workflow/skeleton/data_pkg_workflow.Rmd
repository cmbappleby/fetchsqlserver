---
title: "NCCN Data Package Creation Workflow"
...
This template is for creating a data package and associated metadata required
for DataStore publication using the R package `fetchsqlserver`. The narratives
before code chunks are meant to inform you of what package functions and data
wrangling tools are covered in the code chunks. The template should be used as a
supplement to the documentation.
...
Run the chunk below to install the necessary packages. Even if the packages are
...
Main packages that you need, as well as additional packages that are used in
this file for data wrangling and flattening are in the chunk below.
...
running `fetch_sql_server_data`. This reads both the "overall" and "cust_units"
sheets and saves the information for various functions to use.
...
The data table SamplingEventsAndConditions does not exist because it is two
tables combined into one. The chunk below adds the table description, but this
can be done several different ways. Alternatively, you can wait until the data
names and descriptions Excel spreadsheet (*data_names_descriptions.xlsx*) is
saved when you run `save_data_pkg()` and edit the spreadsheet. The table name
for WQData needs to be changed as well; however, this will be done right before
saving the data package. That way, the table name does not need to be updated in
*field_unit_dict.csv*.
...
Only one Water Quality data table needs flattening, WaterChemistryData.
...
`missing_values()` which fields in a data frame have missing values by only
passing in the data table data frame). To add the generic missing value
...
-   Custom units
    ...
    -   If applicable, `read_data_pkg_info()` or `read_overall_sheet()` saves
        this file
...
ensures all necessary core metadata files are present, converts *keywords.txt*
and *personnel.txt* to TSVs (tab-separated value text files), reads the
...
CUI, and creating a DataStore reference is put together using
...
package folder path should be used as the input folder path because everything
needs to be in one folder because the XML file will be checked against the
contents of the data package folder (data tables, attribute tables, catvar
tables, and metadata files). During the data package creation process you were
likely prompted to select the data package folder; that is where
`write_eml_xml()` will look for the files and save the XML file to. Many
messages will be produced when the EML is checked. This is how you check if your
schema is valid:
...
```{r congruence_checks}
data_pkg_folder <- "C:/Users/cappleby/OneDrive - DOI/Documents/WQ/data_pkg"
